...
def decode_cluster_hierarchy(h:list[list])->list[dict]:
    ...
    for ix, c in enumerate(h):
        n = c[1] - c[0] + 1
        cluster = { '_id': ix, 'L': c[0], 'H': c[1], 'members': n, 'parent':[], 'children':[], }
    ...
    for ix, c in enumerate(clusters):
        ...
        clusters[ix]['parent'] = min(c['parent']) \
            if len(c['parent']) > 0 \
            else None
    ...
    for ix, c in enumerate(clusters):
        if len(c['children']) > 0:
            ...
            clusters[ix]['node_label'] = chr(ord('A') + n_parents - n_parents_traversed - 1)
...
def chart_cluster_hierarchy(h:list[list]|None=None,clusters:list[dict]|None=None,save_file_name:str|None=None)->list:
