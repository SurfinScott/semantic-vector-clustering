...
def load_demo_vectors(
    ...
    print(
        f"Removed {res.deleted_count} documents in {vector_db_name}.{VECTOR_COLLECTION} before loading demo vectors"
    )
    vectors = get_embeddings_vectors(TEXTS, model=EMBEDDING_MODEL)
    assert len(vectors) == len(TEXTS)
    docs = []
    docs.extend(
        [{"text": text, "embedding": vectors[ix]} for ix, text in enumerate(TEXTS)]
    )
